...
apm_options() {
  if [ -n "${JAVA_ENABLE_APM:-}" ] || [ -n "${JAVA_APM_ENABLE:-}" ] ||  [ -n "${JAVA_APM:-}" ]; then
    echo "-javaagent:./skywalking-agent/skywalking-agent.jar -Dskywalking.agent.application_code=$(echo ${HOSTNAME} | cut -d- -f 1,2)"
  fi
}
...
calc_max_memory() {
  ...
  if [ -n "${JAVA_MAX_MEM_RATIO:-}" ]; then
    ...
    calc_mem_opt "${CONTAINER_MAX_MEMORY}" "60" "mx"
  fi
}
...
calc_init_memory() {
  ...
  if [ -z "${JAVA_INIT_MEM_RATIO:-}" ] || [ -z "${CONTAINER_MAX_MEMORY:-}" ] || [ "${JAVA_INIT_MEM_RATIO}" -eq 0 ]; then
    calc_mem_opt "${CONTAINER_MAX_MEMORY}" "60" "ms"
    #return
  fi

  # Calculate Xms from the ratio given
  #calc_mem_opt "${CONTAINER_MAX_MEMORY}" "70" "ms"
}
...
java_options() {
  ...
  # of this awk idiom
  echo "${JAVA_OPTIONS:-} ${JAVA_OPTS:-} $(run_java_options) $(debug_options) $(apm_options) $(proxy_options) $(java_default_options) -Dserver.port=8080 -Dmanagement.port=8081 -Dlog.home=/opt/app/logs  -Dfile.encoding=UTF8 " | awk '$1=$1'
}
