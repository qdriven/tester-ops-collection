...
class ABDExecutor():
    ...
    def __init__(self, device_id=""):
        if device_id == "":
            ...
            self.device_commands = "-s %s".format(device_id)
    ...
    def adb(self, args):
        ...
        :return:
        """
        cmd = "%s %s %s" % (ADB_SHELL, self.device_commands, str(args))
        return self.exec_adb_cmd(cmd)
    ...
    def shell(self, args):
        cmd = "%s %s shell %s" % (ADB_SHELL, self.device_commands, str(args),)
        return self.exec_adb_cmd(cmd)
...
def get_device_list():
    ...
    for device in utils.cmd('adb devices').stdout.readlines():
...
def set_device_yaml():
    ...
    for device in get_device_list():
        adb = ABDExecutor(device)
        utils.Logging.success(
    ...
    ini = utils.ConfigIni()
    with open(ini.get_ini('test_devices', 'device'), 'w') as f:
